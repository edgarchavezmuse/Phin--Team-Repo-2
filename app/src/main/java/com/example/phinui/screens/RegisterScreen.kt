    ...
    Column(
        ...
        Column(
            ...
            if (!isEmailValid && email.isNotBlank()) {
                Spacer(modifier = Modifier.height(6.dp))
                Text(
                    text = "Use a CSUCI email (@myci.csuci.edu)",
                    color = NavText,
                    fontSize = 14.sp
                )
            }
                    ...
                    auth.createUserWithEmailAndPassword(cleanEmail, password)
                        .addOnCompleteListener { authTask ->
                            if (!authTask.isSuccessful) {
                                loading = false
                                errorMessage =
                                    authTask.exception?.message ?: "Registration failed."
                            ...
                            if (uid == null) {
                                loading = false
                                errorMessage = "User created, but no UID was returned."
                                return@addOnCompleteListener
                            }
                            ...
                            db.collection("users")
                                ...
                                .addOnFailureListener { e ->
                                    loading = false
                                    errorMessage =
                                        e.localizedMessage ?: "Profile save failed."
                                }
                        }
                },
                modifier = Modifier.fillMaxWidth(),
